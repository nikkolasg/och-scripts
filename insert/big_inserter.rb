    ...
    class BigSourceInserter < GenericSourceInserter
        ...
        def bulk_insert file
            ...
            sql = "LOAD DATA CONCURRENT LOCAL INFILE '#{tmp}' " + 
                ...
                RubyUtil::sqlize(sql_fields.keys,no_quote:true)
            Database::Mysql::send_raw_sql_cmd sql
            File.delete(tmp) if File.exists?(tmp)
        end

        def write_to_file fields, record
            ...
            @tmp_file.print record.values_at(*fields.values).join("\t")
            @tmp_file.print "\n"
        end
